...
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     2019-08-26 14:29:33                          */
/*==============================================================*/
...
create table client
(
   cliid                varbinary(0) comment '客户编号',
   clitypeid            varbinary(0) comment '客户类别Id',
   cliname              varbinary(0) comment '客户全称',
   clishortname         varbinary(0) comment '客户简称',
   cliaddress           varbinary(0) comment '地方(具体)',
   cliarea              varbinary(0) comment '地区',
   cliename             varbinary(0) comment '英文全称',
   cliprincipal         varbinary(0) comment '负责人',
   clilinkman           varbinary(0) comment '联系人',
   cliphoneone          varbinary(0) comment '联系电话一',
   cliphonetwo          varbinary(0) comment '联系电话二',
   cliphonethree        varbinary(0) comment '联系电话三',
   clibankaccounts      varbinary(0) comment '银行账号',
   cliopenbank          varbinary(0) comment '开户银行',
   empid                varbinary(0) comment '业务人员',
   climobilephone       varbinary(0) comment '移动电话',
   cliemail             varbinary(0) comment '电子邮件',
   cliweb               varbinary(0) comment '网址',
   clifaxes             varbinary(0) comment '传真号码',
   clioldoutdate        date comment '最初销售出货日',
   clioldintodate       date comment '最初销售退货日',
   clinewoutdate        date comment '最近销售出货日',
   clinewintodate       date comment '最近销售退货日',
   clidiscount          varbinary(0) comment '折数(%)',
   clipricestep         varbinary(0) comment '售价等级',
   clicreatedate        date comment '客户建立日',
   clistopdate          date comment '终止交易日',
   clizhangkuanedu      varbinary(0) comment '账款额度',
   cliremainedu         varbinary(0) comment '剩余额度',
   clicondition         varbinary(0) comment '收款条件（天）',
   climonthreckoning    date comment '每月结账日',
   clicreditstep        varbinary(0) comment '信用等级',
   cliqcpremoney        int comment '期初预收款',
   cliqcdealmoney       int comment '期初应收款',
   cliqmpremoney        int comment '期末预收款',
   cliotherremark       varbinary(0) comment '备注',
   clitrash             int comment '是否废码',
   safetyone            varbinary(0) comment '保留字段',
   safetytwo            varbinary(0) comment '保留字段二',
